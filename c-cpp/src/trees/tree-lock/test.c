...
void *test2(void *data)
{
  ...
  int main(int argc, char **argv)
  {
	...
	break;
      case 'h':
	printf("k/v store -- RCU stress test "
	       "(citrus tree)\n"
	       "\n"
	       "Usage:\n"
	       "  intset [options...]\n"
    ...
    initURCU(nb_threads); // initialize RCU with specific numthreads
    set = init(); // initialize the tree
    stop = 0;
